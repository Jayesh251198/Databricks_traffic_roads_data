...
def write_Traffic_Data(StreamingDF,environment):
    ...
    write_Stream = (StreamingDF.writeStream
                    .format('delta')
                    .option("checkpointLocation",checkpoint_path + '/rawTrafficLoad/Checkpt')
                    .outputMode('append')
...
def write_Road_Data(StreamingDF,environment):
    ...
    write_Data = (StreamingDF.writeStream
                    .format('delta')
                    .option("checkpointLocation",checkpoint_path + '/rawRoadsLoad/Checkpt')
                    .outputMode('append')
...
print(f"Resolved catalog path: `{env}_catalog`.`bronze`.`raw_traffic`")
